...
//  correct();
  ...
  // load peak locations of neighboring positions
  loadNeighborPeaks(&pose);
  
  // compute weights
  computeWeights(&pose, image);
//  Serial.print("00: "); Serial.print(pose.weights[0]); Serial.print(", 10: "); Serial.print(pose.weights[1]); Serial.print(", 11: "); Serial.print(pose.weights[2]); Serial.print(", 01: "); Serial.println(pose.weights[3]);
//  pose.weights[0] = 0.25;
//  pose.weights[1] = 0.25;
//  pose.weights[2] = 0.25;
//  pose.weights[3] = 0.25;  
  ...
  computeLocation(&pose);
    
  // reproject pose estimate into image
  predictPeaks(&pose, peak_predict);
  Serial.print('a'); Serial.write(image, 612);
  print_value('b', local_angle_deg);
  print_value('c',peak_predict[0]);
  print_value('d',peak_predict[1]);
  print_value('e',peak_predict[2]);
  print_value('f',peak_predict[3]);
//  Serial.print("00: "); Serial.print(peak_predict[0]); Serial.print(", 10: "); Serial.print(peak_predict[1]); Serial.print(", 11: "); Serial.print(peak_predict[2]); Serial.print(", 01: "); Serial.println(peak_predict[3]);
//  
  // rotate in new position to find best fit
  pose.delta_old = refinePeaks(image, peak_predict, 20);
  
//  Serial.print(", delta:"); Serial.println(pose.delta_old);
  
  // move interpolation window if needed
  int shift = updateNeighbors(&pose);
  
  // compute true angle based on measurement and estimate
  updateAngle(&pose, local_angle_deg);
//  Serial.print("raw: "); Serial.print(angle_deg_raw); Serial.print(", local "); Serial.print(local_angle_deg); Serial.print(", offset: "); Serial.print(pose.offset); Serial.print(", angle2: "); Serial.println(offset2Degree(pose.offset));
//  
  // print results
//  Serial.print("delta: "); Serial.print(pose.delta_old); Serial.print(", Shift: "); Serial.print(shift); Serial.print(", x: "); Serial.print(pose.location[X]); Serial.print(", y: "); Serial.print(pose.location[Y]); Serial.print(", a: "); Serial.println(pose.angle_deg);
//  Serial.print(pose.delta_old); Serial.print(" "); Serial.print(local_angle_deg); Serial.print(" "); Serial.println(pose.angle_deg);
//  print_value(SENSOR_POSE_X, pose.location[X]);
//  print_value(SENSOR_POSE_Y, pose.location[Y]);
//  print_value(SENSOR_POSE_ANGLE, pose.angle_deg);  
  delay(100);
}
